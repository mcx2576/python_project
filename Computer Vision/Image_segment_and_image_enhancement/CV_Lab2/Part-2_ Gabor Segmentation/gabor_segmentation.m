...
for jj = 1 : length(gaborFilterBank)
    ...
    real_out = imfilter(double(img_gray), gaborFilterBank(jj).filterPairs(:,:,1), 'replicate');
    imag_out = imfilter(double(img_gray), gaborFilterBank(jj).filterPairs(:,:,2), 'replicate');
...
if smoothingFlag
    ...
    for i = 1:length(featureMags)
        features(:,:,i) = imgaussfilt(featureMags{i}, 3*gaborFilterBank(i).sigma);
    end
else
...
for i = 1:num_features
    % across each dimension of the data, do standardization
    mean_dim_i = mean(features(:,i));
    std_dim_i = std(features(:,i)); 
    features(:,i) = (features(:,i) - mean_dim_i) ./ std_dim_i;
end
