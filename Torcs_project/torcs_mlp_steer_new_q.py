...
# define the NN
num_features = 22
num_nodes_hidden = 150
num_output = 1
torcs_NN_steer = torcs_mlp_NN_steer(num_features, num_nodes_hidden, num_output)

print(torcs_NN_steer)
...
training_data_filenames = ['train_data1.csv','train_data2.csv','train_data4.csv','train_data5.csv','train_data6.csv','train_data7.csv','train_data8.csv']#['data_track1.csv', 'data_track2.csv']
...
#            data_x.append([float(row[6])])
            data_x.append([float(s) for s in row[5:27]])
#            data_x.append([float(row[5]),float(row[6]),float(row[7])])
...
val_data_filenames = ['val_data1.csv','val_data2.csv']#['data_track1.csv', 'data_track2.csv']
...
#       0    1     2    3    4    5      6     7      8    ...   17    ...   26
#DATA: acc brake steer gear rpm speed dist_c angle edge[0] ... edge[9] ... edge[18]
            val_data_y.append([float(row[2])])
            val_data_x.append([float(s) for s in row[5:27]])
#            val_data_x.append([float(row[5]),float(row[6]),float(row[7])])
...
for i in range(0, num_iter):
    ...
    if i <= 1:
        ...
        learning_rate = 0.005
    optimizer = torch.optim.Adam(torcs_NN_steer.parameters())
    ...
    for n in range(0, num_data_points):
        ...
        output_y = torcs_NN_steer(input_x)
...
torch.save(torcs_NN_steer.state_dict(), "torcs_NN_trained_steer_new_q")

#
torcs_NN_steer2 = torcs_mlp_NN_steer(num_features, num_nodes_hidden, num_output)
torcs_NN_steer2.load_state_dict(torch.load("torcs_NN_trained_steer_new_q"))
